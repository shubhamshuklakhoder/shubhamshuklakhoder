...
export default function PublicProfile() {
  ...
  const fetchProfile = async () => {
    try {
      const response = await axios.get(`${API_URL}/p/${username}`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6" data-testid="error-desc">
            The profile you're looking for doesn't exist or has been removed.
          </p>
          <Link to="/">
            <Button data-testid="go-home-btn" className="bg-blue-600 hover:bg-blue-700 text-white">
              Go to JobLink
            </Button>
          </Link>
        </div>
      ...
      <main className="max-w-md mx-auto min-h-screen bg-white md:my-8 md:rounded-xl md:border md:border-gray-200 md:shadow-sm md:min-h-0">
        <div className="p-6 md:p-8" data-testid="public-profile">
          ...
          <div className="flex justify-center mb-6">
            <div 
              className="w-24 h-24 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-3xl font-bold"
          ...
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold text-gray-900 mb-1" data-testid="profile-name">
              {profile.name}
            </h1>
            {profile.title && (
              <p className="text-gray-600" data-testid="profile-title">{profile.title}</p>
            )}
          </div>

          {/* Bio */}
          {profile.bio && (
            <p className="text-gray-600 text-center mb-8 leading-relaxed" data-testid="profile-bio">
              {profile.bio}
            </p>
          )}
